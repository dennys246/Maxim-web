...
**Status:** Shell plan, drafted 2026-09-02; revised the same day with the hosting, seeding, agent-select and Party Mode decisions; revised again after a six-track code audit of pymaxim 1.1.2 and maxim-pulse (§ The pymaxim tangent, § The maxim-pulse tangent).
...
- **Adventure in the Console is watched, not played.** `MaximHandle._run_sim` forces interactive mode OFF and the DM runtime's human-choice path is stdin-only, so the persistent Maxim picks every choice and the visitor sees scenes and responses stream by. That is still a good demo of the substrate acting, but the page must say "watch Maxim play" until P16 lands.
...
1. **Hosted sandbox (default).** Bundled `smollm-1.7b-instruct` on our CPU, or a cheap cloud model on our key with a hard per-session cap (open question 1). Free, capped, throwaway. The status widget is honest about capacity and queue.
...
- **Broker**: one small Node service. `GET /status`, `POST /session` (Turnstile, per-IP rate limit, capacity gate, seed choice), `GET /session/<id>`, `DELETE /session/<id>` (calls P5 first), and the `/s/<id>/*` HTTP + WebSocket proxy with bearer check. Returns 403 for `/api/setup/mesh`, `/api/setup/cloud` (when our-key mode is on), `/api/probe` and `/api/diagnose` regardless of what the engine does, so the guard is belt and braces with P4.
...
**Status 2026-09-03.** P1, P2, P3, P4, P4b, P4c, P4d, P6, P7 and the `finally` half of P5 landed in pymaxim on branch `feat/sandbox-launch-blockers` (bugs ledger D69–D74; no wire change, OpenAPI snapshot byte-identical). P5's endpoint moved into the P8b contract batch. Two things the audit missed and that PR notes as still CWD-relative: the agent loop's `state_<run_id>.json` (D15) and the CLI scenario path's `--home-dir` default; both are harmless with CWD=/data/run.
...
| P16 | **Console interactivity for Adventure.** An HTTP-backed `PromptHandler` + a `dm_choice` meta-kind in `_META_KINDS` + `POST /api/choice`, so `handle._run_sim` can pass a handler instead of forcing interactive OFF. Until this lands the visitor watches. Also a `/api/stop` for a running adventure (today a 100-turn campaign runs to completion or until the machine dies). | console/, simulation/dm_runtime.py | medium (~150 LOC) |
...
Sequencing inside F: P20a → P19 → P20 → P18 → P16 (or P16 first if "the visitor plays" matters more than "several Maxims in a scene").
...
|---|---|---|
| 0 Measure | The image (with the `llm.json` tiers override, P1–P3 applied) run by hand on a Fly Machine of the target size. Arena or heist campaign, 10 turns, with SmolLM and with a cheap cloud model. | p50 turn latency ≤ 8 s; cold start to `ready` ≤ 45 s from a stopped machine; cost per 20-min session a few cents. If latency fails: smaller model, then performance CPU, GPU last. |
...
| 3 Console flavour | Sandbox bundle in the image, U9 SessionChip, U11 disclosure, hand-off flow. Turn on the button. | A first-time visitor reaches a running Talk or a watched Adventure in under 90 s without reading docs. |
...
1. **Default narrator: our cloud key or SmolLM?** A cheap cloud model on our key with a hard per-session token cap (P10) and SmolLM as the fallback when the monthly budget is spent gives much better Adventures and a per-turn cost shape instead of idle CPU. Verify current per-token pricing, then decide. Either way P9 must land before any key is used.
2. **Watch or play?** Ship Phase 3 with Adventure watched (honest, zero engine work) or pull P16 forward so the visitor chooses? Recommendation: watch at launch, P16 when the first visitors ask.
3. **Adversarial prompts.** With P4/P4c/P4d the damage is bounded to the session's own machine and budget. Is there any moderation stance beyond that, and can a visitor report a session?
4. **Budget ceiling.** A monthly number, set before launch, that the broker enforces by refusing new sessions.
5. **Opt-in session sharing.** Transcripts are not logged by default. Can a visitor choose to send us one for a bug report? Otherwise we never see what broke.
6. **Contribute back to an Oasis?** Not 1.1 scope. If ever: route through `substrate_merge` only, `strict_geometry=True`, contributor-count floor before publication, episodes ingested but never re-broadcast, and a raw-substrate arm kept for the headline claim (`maxim_hivemind.md` § Confound discipline).
