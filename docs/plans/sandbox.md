...
|---|---|---|---|---|
| P21 | **Prompt budget for the console agent — DONE 2026-09-05 (pymaxim #623, ledger D80–D82).** Measured: the scene-roster `tools` section (a DM `choose` description embeds the current options) sat second in the "stable" prefix and invalidated the ~2.5k tokens behind it — the 559 cached tokens above are exactly the head before it; `coding_guidelines` (a FILE WORKSPACE REMINDER + GLOB PATTERN GUIDE triggered by the word "patterns" in the narration) cost 614 tokens per call for an agent with no file tools; the roster listed 25 tools of which the sandbox allow-list permits ~8. Shipped: phase-scoped sections emitted last in the stable prefix; file guidance and the CWD manifest gated on a file tool being on the roster; the advertised roster filtered through the executor's permission gate; `llm.max_response_tokens` and `llm.deliberation_max_cycles` (env `MAXIM_LLM_MAX_RESPONSE_TOKENS` / `MAXIM_LLM_DELIBERATION_MAX_CYCLES`, read at loop start). Effect on latency is a prediction until the pair re-run below. | agents/prompt_budgeter.py, agents/prompt_builder.py, runtime/executor.py, runtime/agent_loop.py, runtime/config_loader.py | medium | launch |
| P22 | **Native llama.cpp server as the leader's spawn target.** The auto-spawner launches the Python `llama_cpp.server` (single slot, no prompt cache); the sandbox needs `llama-server` with parallel slots, prompt caching and Metal. Add a spawner backend choice (`auto_spawn.server = llama_cpp \| llama_server`) with `-np`, `-c`, `-ctk/-ctv`, `--cache-reuse` from config, and keep the existing reuse-if-alive path. **Add slot pinning** (§ P21 re-run): the proxy forwards a stable `id_slot` per session, or the spawner sizes `-np` so narrator calls never share the agent's slot — the 28 s vs 48 s solo swing is this. | runtime/local_server_spawner.py, runtime/lane_backends.py, config_loader | small | Phase 1 (until then: run llama-server by hand, `auto_spawn.llm_server=false`) |
...
## P21 re-run (2026-09-05)

Same apparatus as Phase 0, engine `main@ed6df4ce` (pymaxim #623 merged), from-source llama-server with Metal, `-np 4 --cache-reuse 256`, arena campaign, 5 encounters. The harness now prints the engine commit it measured: the first re-run of the day reported a 28 s solo turn that turned out to be a **stale Docker layer** (the cached `git clone` from before the merge) — the image tag now carries the resolved commit, and that number is retracted.

| Arm | Sessions | Per-call p50 | Steady turn p50 | Per-call trace |
|---|---|---|---|---|
| Phase 0 baseline (2026-09-04) | 1 | 63 s | 68 s | 4,678 tokens in; 559 cached after an encounter change, 3,450 on a same-encounter call |
| P21, default cycles | 1 | 38.5 s | 48 s | **4,174 in** (the file guidance is gone); **2,041 cached** after an encounter change (the head before the roster), 3,450 same-encounter |
| P21, `deliberation_max_cycles=1` + the sandbox allow-list | 1 | 22.3 s | 28 s | 4,174 in — the allow-list changed nothing (see below); 3,450–3,750 cached on **every** call including encounter changes |
| Phase 0 baseline | 4 | 101 s | 222 s | |
| P21, `deliberation_max_cycles=1` | 4 | 92.6 s | 168 s | exactly 5 large calls per session (one per encounter — the cap works); 2,041 cached after encounter changes, 75–250 s per call under contention |

What it says:

- **The ordering fix does what it was built to do.** An encounter change now keeps 2,041 tokens cached instead of 559, and a call costs ~40 s instead of ~65 s. Per-call input fell by ~500 tokens. Solo steady turn 68 s → 48 s on identical cache luck.
