...
The library is organized into **{data.categories.length} categories**. Every component carries one or more genre tags (or is genre-neutral), which drives genre gating during a campaign. The counts below are generated from the registry files in **{data.source_label}** — see [the count, derived](/reference/components/#the-count-derived) for the counting rule and why older hand-typed totals (72, 73, 81) disagree.
...
The full per-component listing — every entity, its sensors, affordances, and failure modes — lives on the **[Components reference](/reference/components/)**, a searchable catalog with category and genre filters that is regenerated from the registry rather than maintained by hand. The canonical source is the registry itself: [`src/maxim/_data/components/`](https://github.com/dennys246/Maxim/tree/main/src/maxim/_data/components) in the engine repo, described in [docs/user/component-library.md](https://github.com/dennys246/Maxim/blob/main/docs/user/component-library.md).

To give a sense of the shape: `weapons/magic_staff` (fantasy) tracks mana, durability, attunement, and corruption, and can fail via `mana_exhaustion`, `corruption_overload`, or `shattered`. `creatures/dragon` (fantasy) tracks hp, fire-breath charge, aggression, altitude, and armor integrity, failing to `grounded`, `breath_exhausted`, or `death`. `bodies/reachy_mini` (a robot body) tracks head yaw/pitch/roll, camera health, battery, and motor temperature, failing to `thermal_throttling`, `low_battery`, `pose_drift`, or `camera_lost`. The Items category also bundles the `cradle_*` scene objects used by the [Cradle](/research/cradle/) developmental arc, reachable through the registry like any other component.
...
The recognised genres are `fantasy`, `cyberpunk`, `scifi`, `horror`, `historical`, `modern`, and `devops`. Explicit refs bypass the gate — `ref: "npcs/corpo_guard"` loads in a fantasy campaign if you ask for it by name; only generative lookups and registry queries are filtered. How campaigns declare a genre is on [DM campaigns](/guides/dm-campaigns/#genre-gating).
...
The genre tag in `tags` (here `fantasy`) is what genre gating reads. Once written, the ComponentIndex adds it to the alias table and embeds its description, so it becomes reachable both by exact path (`weapons/my_weapon`) and by semantic query.
...
- [DM campaigns](/guides/dm-campaigns/) and [Simulation](/guides/simulation/) — where components are cast into campaigns as NPCs, world objects, and encounter props.

## Going deeper

- [Components reference](/reference/components/) — the searchable catalog, every component with its sensors, affordances, and failure modes.
- Repository: [github.com/dennys246/Maxim](https://github.com/dennys246/Maxim); registry at [`src/maxim/_data/components/`](https://github.com/dennys246/Maxim/tree/main/src/maxim/_data/components).
- User doc: [docs/user/component-library.md](https://github.com/dennys246/Maxim/blob/main/docs/user/component-library.md) — its category tables are hand-maintained; when they and the registry disagree, the files win.
