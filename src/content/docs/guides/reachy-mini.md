...
Credit is **relief** — error reduction per step — not raw pain; an earlier simulation study showed pain alone cannot drive action selection. On hardware, direction learning went from chance to 100% correct within about ten trials, with a visible one-trial correction: an exploratory wrong turn on trial 2 made the error worse, and on the very next visit to that state the greedy choice had already flipped. Reloading the persisted substrate in a later session started at 100% correct on trial 0. A third arm merged two independently trained substrates into a working combined policy; that arm was downgraded on 2026-09-01 to a vacuous guard — its gauntlet passed with the merge replaced by `return left`, and the two learners shared one agent id and one cluster space, so it did not test sharing between independent agents (see the [evidence page](/research/evidence/#sensorimotor-learning-on-real-hardware)). The learned policy is eight numbers with provenance and zero personal data.
