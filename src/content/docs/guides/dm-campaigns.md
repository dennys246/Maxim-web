...
`--dm` is also accepted, but it does less than its help text suggests. With a YAML path it is redundant — a campaign is auto-detected from the `campaign:` and `encounters:` keys. With a goal string (`maxim --sim "run a heist" --dm`) it starts the **generative narrative campaign** runner, the same one a plain `--sim "<goal>"` uses (see [Generative campaigns](/guides/simulation/generative/)), with `dm` recorded as the flow-shape label in reports and logs. It does not author a campaign: the design in which an architect agent writes campaign YAML and hands it to the DM runtime is **not implemented** in 1.1.2. The CLI help used to overstate this — 1.1.1's help promised "generate a campaign" — but 1.1.2 rewrote it to say plainly that `--dm` "does NOT author campaign YAML (the architect path is not implemented)".
