...
There is also a shorthand, `maxim --benchmark [tier1|tier2|tier3|all] --models …`, which picks a suite for you. In 1.1.0 only the `tier1`/`all` mapping points at a file that exists (`cognitive_suite.yaml`); `tier2` and `tier3` map to `biosystem_suite.yaml` and `embodiment_suite.yaml`, which are not in the tree. Use `--sim benchmark --campaign <path>` and name the suite explicitly.
...
All flags above are present in the 1.1.0 wheel's `maxim --help`.
...
The runner has a hook for a third tier of embodiment metrics, collected automatically if the run's introspector exposes an `embodiment_stats()` method. In 1.1.0 nothing does, so the hook returns nothing and no Tier 3 metric appears in a report. Descriptions of Tier 3 metrics elsewhere describe a design, not a measurement.
