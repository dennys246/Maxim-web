...
- **Entorhinal Cortex → Pattern Completion.** (Since 1.1.3 completion is geometry-aware: nodes encoded in a different space are skipped rather than scored — see the [entorhinal cortex page](/systems/entorhinal-cortex/#concept-decomposition).) The EC's grid cells create abstract representations that generalize across experiences — letting you recognize that "finding a water bottle in the kitchen" is similar to "finding a mug in the kitchen." The MemoryAgent uses the EC's similarity search to find analogous situations even when surface details differ, enabling transfer learning from past experience to novel contexts. (That search is LSH-*structured* but, as shipped, a single-bucket exact scan — see [memory & consolidation](/memory/overview/) and defect D51.)
