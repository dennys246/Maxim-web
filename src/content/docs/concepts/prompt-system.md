...
Order inside that prefix matters as much as membership, because a prefix cache dies at the first differing byte. On engine main since 2026-09-05 (after 1.1.4) the tool manifest is marked *phase-scoped* and emitted last: it is stable within a scene but changes at every encounter (a campaign's `choose` tool lists the current options in its description), and when it sat second in the prefix a scene change threw away everything behind it — the sandbox measurement that motivated the change saw 559 of 3,450 cacheable tokens survive. Two smaller trims landed with it: the workspace manifest and the file and glob guidance are composed only when a file tool is actually on the roster, and the roster itself lists only tools the executor's permission gate would let through.
